...
def getf (link):
    ...
    print (os.getcwd())
    gendoxy = 'cp ' + dir_path+'/Doxyfile ' + os.getcwd()
    os.system(gendoxy)
    cmd = 'doxygen Doxyfile'
    os.system(cmd)
    os.chdir(os.getcwd() + "/xml")
    os.system("rm index.xml")
    os.system('ls')
    thank(nme)
    return
...
def index(request):
    template = loader.get_template('soviz/index.html')
    return HttpResponse(template.render({'1':1}, request))

def repo(request):
    template = loader.get_template('soviz/repos.html')
    return HttpResponse(template.render({'form':form}, request))

def get_repo(request):
    if request.method == 'POST':
        form = RepoForm(request.POST)
        url = request.POST['repo']
        print("===================================================================\n")
        print(url)
        print("===================================================================\n")
        getf(str(url))
...
def th(request):
    template = loader.get_template('soviz/sub.html')
    return HttpResponse(template.render({'3':1}, request))
